...
export class DataBridge {
    constructor(options = {}) {
        ...
        // Listen for postMessage from parent (demo pages). Off by default on
        // the website: the brain mounts inline, no iframe parent exists.
        this._messageHandler = null;
        if (options.listenPostMessage) {
            this._messageHandler = (e) => this._handlePostMessage(e);
            window.addEventListener('message', this._messageHandler);
        }
    }

    /** Register a callback for connection-status changes: fn('live'|'simulated'|'connecting'|'disconnected') */
    onStatus(fn) {
        this.statusListeners.push(fn);
    }

    /** Tear down: close socket, stop loops, remove global listeners. */
    dispose() {
    ...
    _updateConnectionStatus(status) {
        for (const fn of this.statusListeners) {
            try { fn(status); } catch (e) { console.error('[DataBridge] Status listener error:', e); }
        }
